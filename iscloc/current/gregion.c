 ...
 */
static int default_depth_from_grid(double lat, double lon, double gres,
 ...
 */
    if ((fp = fopen(fname, "r")) == NULL) {
        fprintf(errfp, "read_FlinnEngdahl: cannot open %s\n", fname);
        fprintf(logfp, "read_FlinnEngdahl: cannot open %s\n", fname);
        errorcode = 2;
        return 1;
    }
/*
 ...
 */
    ...
    if ((fep->nl = (int *)calloc(nlat, sizeof(int))) == NULL) {
        fprintf(logfp, "read_FlinnEngdahl: cannot allocate memory\n");
        fprintf(errfp, "read_FlinnEngdahl: cannot allocate memory\n");
        fclose(fp);
        Free(fep->grn);
        Free(fep->lon);
        errorcode = 1;
 ...
 *  Called by:
 *     eventloc, get_default_depth, put_hypocenter
 *  Calls:
 *     ibracket
 */
int gregnum(double lat, double lon, FE *fep)
{
    ...
    if (n == 2)
        ...
        ibracket(ilon, n, fep->lon[ilat], &ilo, &ihi);
 ...
 *     fep       - FE structure
 *     grn_depth - geographic region default depths
 *  Output Arguments:
 ...
 *     eventloc
 *  Calls:
 *     default_depth_from_grid, gregnum
 */
double get_default_depth(SOLREC *sp, int ngrid, double gres,
                         double **depthgrid, FE *fep, double *grn_depth,
                         int *isdefdep)
{
    double defdep = 0.;
    int grn = 0;
 ...
 */
    if (default_depth_from_grid(sp->lat, sp->lon, gres, ngrid,
                                depthgrid, &defdep)) {
        sp->depfixtype = 5;
        *isdefdep = 1;
 ...
 *  Return:
 *     grn_depth - pointer to default depth vector
 *  Called by:
 *     read_data_files
 *  Calls:
 *     Free
...
{
    FILE *fp;
    double *grn_depth = (double *)NULL;
    double depth = 0.;
    int ngreg = 757, i, k;
/*
 *  memory allocation
 */
    if ((grn_depth = (double *)calloc(ngreg, sizeof(double))) == NULL) {
        fprintf(logfp, "read_default_depth: cannot allocate memory\n");
        fprintf(errfp, "read_default_depth: cannot allocate memory\n");
        return (double *)NULL;
    }
/*
 ...
 */
    if ((fp = fopen(fname, "r")) == NULL) {
        fprintf(errfp, "read_default_depth: cannot open %s\n", fname);
        fprintf(logfp, "read_default_depth: cannot open %s\n", fname);
        Free(grn_depth);
        return (double *)NULL;
    }
    for (i = 0; i < ngreg; i++) {
 ...
 */
    if ((fp = fopen(fname, "r")) == NULL) {
        fprintf(errfp, "read_default_depth_grid: cannot open %s\n", fname);
        fprintf(logfp, "read_default_depth_grid: cannot open %s\n", fname);
        errorcode = 2;
        return (double **)NULL;
    }
/*
 ...
 *  memory allocation
 */
    if ((depthgrid = alloc_matrix(n, 3)) == NULL)
        return (double **)NULL;
/*
